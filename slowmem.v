`timescale 1ns/1ps
// Word-wide backing memory with a fixed access latency. Request is held until
// a one-cycle ack. Initialized with a deterministic pattern for verification,
// optionally overlaid by MEMFILE in simulation.
module slowmem #(
    parameter MEMW = 1<<14,        // words
    parameter LAT  = 8,            // cycles per word access
    parameter [31:0] RAMBASE = 32'h0000_0000,
    parameter MEMFILE = "",
    parameter MEMFILE_WORDS = 0
)(
    input  wire        clk, rst,
    input  wire        m_req,
    input  wire        m_we,
    input  wire [31:0] m_addr,     // byte address (word aligned)
    input  wire [31:0] m_wdata,
    output reg  [31:0] m_rdata,
    output reg         m_ack
);
    reg [31:0] mem [0:MEMW-1];
    reg        busy;
    reg        wait_drop;
    reg [15:0] cnt;
    reg        lat_we;
    reg [31:0] lat_addr;
    reg [31:0] lat_wdata;
    wire [31:0] lat_index = (lat_addr - RAMBASE) >> 2;
    integer k;
    initial begin
        for (k=0;k<MEMW;k=k+1) mem[k] = (k<<2) ^ 32'hA5A5_0000;  // ref pattern
`ifdef SIM_INIT
        if (MEMFILE != "") begin
            if (MEMFILE_WORDS == 0)
                $readmemh(MEMFILE, mem);
            else
                $readmemh(MEMFILE, mem, 0, MEMFILE_WORDS-1);
        end
`endif
        busy=0; wait_drop=0; cnt=0; m_ack=0; m_rdata=0; lat_we=0; lat_addr=0; lat_wdata=0;
    end
    always @(posedge clk) begin
        if (rst) begin
            m_ack<=0; busy<=0; wait_drop<=0; cnt<=0;
            lat_we<=0; lat_addr<=0; lat_wdata<=0;
        end
        else begin
            m_ack <= 1'b0;
            if (wait_drop) begin
                if (!m_req) wait_drop <= 1'b0;
            end else if (!busy) begin
                if (m_req) begin
                    busy <= 1'b1;
                    cnt <= (LAT[15:0] == 16'd0) ? 16'd0 : LAT[15:0]-16'd1;
                    lat_we <= m_we;
                    lat_addr <= m_addr;
                    lat_wdata <= m_wdata;
                end
            end else if (cnt==0) begin
                m_ack <= 1'b1; busy <= 1'b0; wait_drop <= 1'b1;
                if (lat_we) mem[lat_index] <= lat_wdata;
                m_rdata <= mem[lat_index];
            end else cnt <= cnt - 16'd1;
        end
    end
endmodule
