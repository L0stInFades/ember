...
run_step directed bash -lc '
  set -euo pipefail
  for t in isa amotest mmu ctest shtest mtest misalign utrap mprv mxr upage ifault wpfault sum badpte superpage amo_mmu; do
    echo "=== $t ==="
...
run_step trace bash -lc '
  ...
  for t in isa amotest mmu ctest shtest mtest misalign utrap mprv mxr upage ifault wpfault sum badpte superpage amo_mmu; do
    ...
    trap_arg="--no-trap"
    case "$t" in
      mmu|misalign|utrap|mprv|mxr|upage|ifault|wpfault|sum|badpte|superpage|amo_mmu) trap_arg="" ;;
    esac
