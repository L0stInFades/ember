...
run_step directed bash -lc '
  set -euo pipefail
  for t in isa amotest mmu ctest shtest mtest utrap mprv mxr upage ifault wpfault sum badpte; do
    echo "=== $t ==="
...
run_step trace bash -lc '
  ...
  for t in isa amotest mmu ctest shtest mtest utrap mprv mxr upage ifault wpfault sum badpte; do
    ...
    trap_arg="--no-trap"
    case "$t" in
      mmu|utrap|mprv|mxr|upage|ifault|wpfault|sum|badpte) trap_arg="" ;;
    esac
...
run_step reftrace bash -lc '
  ...
  for t in isa amotest mmu ctest shtest mtest utrap mprv mxr upage ifault wpfault sum badpte; do
    ...
    case "$t" in
      mmu|utrap|mprv|mxr|upage|ifault|wpfault|sum|badpte) priv_arg="" ;;
    esac
