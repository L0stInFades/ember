...
- The external gate now also covers `mprv`, `mxr`, `upage`, `ifault`,
  `wpfault`, `sum`, `badpte`, `superpage`, and `amo_mmu` with Spike
  `RV32IMA_Svadu`, a 4 MB memory map, and a dynamic syscon report-store stop
  point. Those comparisons include the directed permission/trap bodies before
...
comparison.
The remaining `mprv` Spike mismatch was traced to the test not enabling
`menvcfgh.ADUE`, while Spike/Svadu requires that bit for hardware A/D updates.
After adding the same ADUE setup used by the other Sv32 directed tests,
`logs/ci-p1-20260629-p1-external-mprv-v2` passed with 17 external tests, 70,670
compared retired rows, 24 compared trap rows, 85,628 Spike commits, and 1
terminal-trap comparison. `logs/ci-evidence-health-20260629-p1-external-mprv-v2`
then passed with 42/42 checks under the new default 17-test P1 external floor;
negative checks for `--min-p1-external-tests 18` and
`--min-p1-external-terminal-traps 2` failed as intended. The local quick profile
...
Source commit `16f0b80` then passed hosted macOS CI as run `28336556293`:
quick `pass=1 fail=0`, retained RVTRACE audit remains green, and
...
retained CSV.
Source commit `d1acc71` then passed hosted macOS CI as run `28339094468`:
quick `pass=1 fail=0`, and `logs/github-p1-external-28339094468` records the
exact 17-test Spike-prefix list
(`isa,amotest,ctest,shtest,mtest,mmu,utrap,misalign,mprv,mxr,upage,ifault,wpfault,sum,badpte,superpage,amo_mmu`)
...
The same ACT/Spike path now retains the exact selected 106-test list as
`test_names`, not only the total, group list, and per-group counts. The smoke
script emits `P1_ACT4_SPIKE_TESTS`, metrics can infer the list from retained
`ACT4_SPIKE_TEST` lines, dashboard top-level JSON and history trend data retain
it, and local `logs/ci-evidence-health-20260629-act4-test-list` passes 98/98
checks. An intentionally equal-length `--require-p1-act4-spike-test-list` with
the final `ZihintntlZca` test name replaced fails in the latest summary,
dashboard, and history checks.
...
regression in 1m52s and P1 external in 10m46s. Its downloaded P1 artifact
...
`python3 tools/import_github_run_artifacts.py 28341807002` imports 2 artifacts and
3 summaries into `logs/github-run-28341807002`, records head SHA
`838e27bdebccacb3659fd7af57048744dcba7150`, and refreshes the dashboard to
101 summaries / 101 history records with latest quick, P1 external, RVTRACE, and
RVTRACE coverage evidence all coming from that hosted run. The default
evidence-health checker passes 117/117 over that self-imported evidence with a
...
streak. Commit `05729a9` then passed hosted CI run `28360409336`: quick regression
in 1m51s and P1 external in 3m19s with the same 17 external tests, 70,670 compared
retired rows, 23 trap-exception checks, one terminal trap, and 9 device
completions. Running `python3 tools/import_github_run_artifacts.py 28360409336`
imports 2 artifacts and 3 summaries into `logs/github-run-28360409336`, records
head SHA `05729a92fd79ffa44a95f8f2f44a8fadcdc11ec1`, and refreshes the dashboard
to 107 summaries / 107 history records with latest quick, P1 external, RVTRACE,
and RVTRACE coverage evidence all coming from that hosted run. The default
evidence-health checker passes 125/125 over that imported evidence with a 38-run
pass streak, and
`logs/ci-evidence-health-20260629-device-complete-github-import` passes 125/125,
refreshing the dashboard to 108 summaries, 108 history records, and a 39-run pass
streak.
...
reports 108 summaries scanned, 108 retained history runs, a 39-run pass streak,
profile counts of `evidence-health=39`, `p0-evidence=2`, `p1=22`,
`p1-trace-audit=23`, and `quick=22`, 22 P1 external evidence runs, 23 RVTRACE
coverage artifact runs, 39 CI evidence health runs, latest P0 Linux evidence from
`logs/ci-p0-evidence-20260629-boot-cycles`, latest imported hosted P1 external
evidence from `logs/github-p1-external-28360409336`, latest imported hosted
RVTRACE audit/coverage from `logs/github-p1-trace-audit-28360409336`, latest CI
evidence health from
`logs/ci-evidence-health-20260629-device-complete-github-import`, and best PnR at
