...
module tb_rvlinux_min_core_translated_mmio;
    reg clk = 1'b0;
    reg rst = 1'b1;
    reg rx_valid = 1'b0;
    ...
    localparam [31:0] UART_PTE   = 32'h0400_0007; // PA 0x10000000, V/R/W
    localparam [31:0] SYSCON_PTE = 32'h0444_0007; // PA 0x11100000, V/R/W
    ...
    initial begin
        ...
        // M-mode setup: install SATP and MRET into S-mode at 0x40.
        ...
        // S-mode code uses virtual MMIO addresses.
        write_word(16, 32'h0000_1237); // lui  x4,0x1       -> VA 0x00001000
        write_word(17, 32'h0420_0293); // addi x5,x0,0x42
        write_word(18, 32'h0052_0023); // sb   x5,0(x4)     -> UART THR
        write_word(19, 32'h0052_4303); // lbu  x6,5(x4)     -> UART LSR
        write_word(20, 32'h0003_0513); // addi x10,x6,0
        write_word(21, 32'h0000_23b7); // lui  x7,0x2       -> VA 0x00002000
        write_word(22, 32'h0000_5437); // lui  x8,0x5
        write_word(23, 32'h5554_0413); // addi x8,x8,0x555 -> 0x5555
        write_word(24, 32'h0083_a023); // sw   x8,0(x7)     -> syscon poweroff
        ...
        if (!halt) begin
            $display("MIN_CORE_TRANSLATED_MMIO_RESULT: FAIL timeout pc=%08h retired=%0d owner=%0d",
                     pc, retired, cluster_active_owner);
            $finish;
        end
        if (fault) begin
            $display("MIN_CORE_TRANSLATED_MMIO_RESULT: FAIL fault cause=%0d tval=%08h pc=%08h retired=%0d priv=%0d",
                     fault_cause, fault_tval, pc, retired, dbg_priv);
            $finish;
        end
        if (dbg_priv != 2'd1 || !uart_seen || uart_byte != 8'h42) begin
            $display("MIN_CORE_TRANSLATED_MMIO_RESULT: FAIL priv=%0d uart_seen=%0d uart_byte=%02h",
                     dbg_priv, uart_seen, uart_byte);
            $finish;
        end
        if (dbg_x6 != 32'h0000_0060 || dbg_x10 != 32'h0000_0060) begin
            $display("MIN_CORE_TRANSLATED_MMIO_RESULT: FAIL x6=%08h x10=%08h",
                     dbg_x6, dbg_x10);
            $finish;
        end
        if (exit_code != 32'd0 || pc != 32'h0000_0064 ||
            retired != 32'd18 || ad_writes < 32'd3) begin
            $display("MIN_CORE_TRANSLATED_MMIO_RESULT: FAIL exit=%0d pc=%08h retired=%0d ad_writes=%0d",
                     exit_code, pc, retired, ad_writes);
            $finish;
        end

        $display("MIN_CORE_TRANSLATED_MMIO_RESULT: PASS retired=%0d pc=%08h priv=%0d uart=%02h ad_writes=%0d",
                 retired, pc, dbg_priv, uart_byte, ad_writes);
        $finish;
    end
endmodule
